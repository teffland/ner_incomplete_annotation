...
class LinearCRF(nn.Module):
    def __init__(self, config, print_info: bool = True):
        ...
        self.start_idx = self.label2idx[config.O]
        self.end_idx = self.label2idx[config.O]
        ...
        # init_transition[self.pad_idx, :] = -10000.0
        self.add_constraint_for_ioblu(init_transition)

        self.transition = nn.Parameter(init_transition).to(self.device)

    def add_constraint_for_ioblu(self, transition: torch.Tensor):
        ...
        for prev_label in self.labels:
            ...
            for next_label in self.labels:
                if next_label == START or next_label == STOP or next_label == PAD:
                    continue
                if prev_label == "O" and (next_label.startswith("I-") or next_label.startswith("L-")):
                ...
                if prev_label.startswith("B-") or prev_label.startswith("I-"):
                    if next_label.startswith("O") or next_label.startswith("B-") or next_label.startswith("U-"):
                        transition[self.label2idx[prev_label], self.label2idx[next_label]] = -10000.0
                    elif prev_label[2:] != next_label[2:]:
                        transition[self.label2idx[prev_label], self.label2idx[next_label]] = -10000.0
                if prev_label.startswith("U-") or prev_label.startswith("L-"):
                    if next_label.startswith("I-") or next_label.startswith("L-"):
                        transition[self.label2idx[prev_label], self.label2idx[next_label]] = -10000.0
        ##constraint for start and end
        for label in self.labels:
            if label.startswith("I-") or label.startswith("L-"):
    ...
    def forward(self, lstm_scores, word_seq_lens, tags, mask, marginals=None):
        ...
        labeled_score = (
            self.forward_labeled(all_scores, word_seq_lens, tags, mask)
            if marginals is None
            else self.forward_labeled_with_marginal(all_scores, word_seq_lens, marginals)
        )
    ...
    def forward_labeled_with_marginal(
        self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor, marginals: torch.Tensor
    ) -> torch.Tensor:
        """
        Calculate the scores with the forward algorithm. Basically calculating the normalization term
    ...
    def forward_labeled(
        self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor, tags: torch.Tensor, masks: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        currentTagScores = torch.gather(
            all_scores, 3, tags.view(batchSize, sentLength, 1, 1).expand(batchSize, sentLength, self.label_size, 1)
        ).view(batchSize, -1, self.label_size)
        if sentLength != 1:
            tagTransScoresMiddle = torch.gather(
                currentTagScores[:, 1:, :], 2, tags[:, : sentLength - 1].view(batchSize, sentLength - 1, 1)
            ).view(batchSize, -1)
        ...
        tagTransScoresEnd = torch.gather(
            self.transition[:, self.end_idx].view(1, self.label_size).expand(batchSize, self.label_size), 1, endTagIds
        ).view(batchSize)
    ...
    def decode(
        self, lstm_scores: torch.Tensor, sent_len: torch.Tensor, annotation_mask: torch.Tensor = None
    ...
    def constrainted_viterbi_decode(
        self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor, annotation_mask: torch.Tensor = None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        for wordIdx in range(1, sentLength):
            ...
            scoresRecord[:, wordIdx, :] = torch.gather(
                scoresIdx, 1, idxRecord[:, wordIdx, :].view(batchSize, 1, self.label_size)
            ).view(batchSize, self.label_size)

        lastScores = torch.gather(
            scoresRecord, 1, word_seq_lens.view(batchSize, 1, 1).expand(batchSize, 1, self.label_size) - 1
        ).view(
        ...
        for distance2Last in range(sentLength - 1):
            lastNIdxRecord = torch.gather(
                idxRecord,
                1,
                torch.where(word_seq_lens - distance2Last - 1 > 0, word_seq_lens - distance2Last - 1, mask)
                .view(batchSize, 1, 1)
