...
class SlantedTriangular(LearningRateScheduler):
    ...
    def get_values(self):
        ...
        # FIXME: This appears to be broken in original, had to change to >2 from >1 to fix
        # I guess .step gets called twice before actually training?
        if len(self.batch_num_total_epoch_end) > 2:
            ...
            if not self.printed_num_steps:
                print("ACTUAL NUM STEPS PER EPOCH", actual_num_steps_per_epoch)
                self.printed_num_steps = True
        else:
