...
def simple_batching(config, insts: List[Instance]) -> Tuple:
    ...
    pad_idx = config.word2idx[config.PAD]  # fill with pad tokens
    word_seq_tensor = pad_idx * torch.ones((batch_size, max_seq_len), dtype=torch.long)
    ...
    return word_seq_tensor, word_seq_len, annotation_mask, marginals, label_seq_tensor
...
def get_optimizer(config: Config, model: nn.Module):
    ...
    if config.optimizer.lower() == "sgd":
        ...
        print(
            colored(
                "Using Adam: lr is: {}, L2 regularization is: {}".format(config.learning_rate, config.l2), "yellow"
            )
        )
        return optim.Adam(params, lr=config.learning_rate, weight_decay=float(config.l2))
    else:
