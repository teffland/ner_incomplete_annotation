...
import time
from model.adapted_neuralcrf import NNCRF

# from config.slanted_triangular import SlantedTriangluar
import torch
from typing import List
from common import Instance
from termcolor import colored
import os
from config.adapted_utils import load_elmo_vec
import pickle
import tarfile
import shutil
import math
...
def parse_arguments(parser):
    ...
    parser.add_argument(
        "--device", type=str, default="cpu", choices=["cpu", "cuda:0", "cuda:1", "cuda:2"], help="GPU/CPU devices"
    )
    parser.add_argument("--seed", type=int, default=42, help="random seed")
    # parser.add_argument('--digit2zero', action="store_true", default=True,
    #                     help="convert the number to 0, make it true is better")
    parser.add_argument("--train-file", type=str)  # , default="conll2003")
    parser.add_argument("--dev-file", type=str)  # , default="conll2003")
    parser.add_argument("--test-file", type=str)  # , default="conll2003")
    ...
    parser.add_argument(
        "--num_outer_iterations", type=int, default=10, help="Number of outer iterations for cross validation"
    )
    parser.add_argument("--drop-unannotated-sentences", action="store_true")
    parser.add_argument("--drop-unannotated-docs", action="store_true")

    ##model hyperparameter
    parser.add_argument(
        "--variant", type=str, default="soft", choices=["hard", "soft"], help="The hard or soft variant of the model"
    )
    ...
    parser.add_argument("--bert-model", type=str, help="transformers model name to use")
...
def train_model(
    config: Config, train_insts: List[List[Instance]], dev_insts: List[Instance], test_insts: List[Instance]
):
    train_num = sum([len(insts) for insts in train_insts])
    print("[Training Info] number of training instances: %d" % (train_num))
    print("[Training Info] number of dev instances: %d" % (len(dev_insts)))
    print("[Training Info] number of test instances: %d" % (len(test_insts)))
    ...
    for iter in range(num_outer_iterations):
        ...
        for fold_id, folded_train_insts in enumerate(train_insts):
            ...
            model_name = model_folder + f"/bert_crf_{fold_id}.m"
        ...
        model_name = model_folder + "/final_bert_crf.m"
        config_name = model_folder + "/config.conf"
        res_name = res_folder + "/bert_crf.results".format()
...
def train_one(
    ...
    lr_scheduler = SlantedTriangular(optimizer, config.num_epochs, num_steps_per_epoch=len(train_batches), ratio=16)
    ...
    for i in range(1, epoch + 1):
        ...
        for index in tqdm(np.random.permutation(len(train_batches)), f"Training epoch {i}", len(train_batches)):
            model.train()
        ...
        end_time = time.time()
        print(
            "Epoch %d: %.5f, Time is %.2fs" % (i, epoch_loss / len(train_batches), end_time - start_time), flush=True
        )
...
def main():
    ...
    reader = Reader(
        conf.bert_model,
        drop_unannotated_docs=opt.drop_unannotated_docs,
        drop_unannotated_sentences=opt.drop_unannotated_sentences,
    )
    ...
    conf.build_word_idx(reader.subword_converter.tokenizer)
